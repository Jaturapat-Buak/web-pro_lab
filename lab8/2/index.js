...
app.get("/", (req, res) => {
  res.send(
    '<a href = "/create1">Create Table</a><br><a href = "/insert1">Insert Data</a><br><a href = "/showdata">Show Data</a><br><a href = "/login">login</a>',
  );
});
...
app.post("/signin", (req, res) => {
  ...
  conn.query(sql, [login, login], (err, result) => {
    if (err) throw err;

    if (result.length === 0) {
      return res.render("login", { message: "ไม่พบบัญชีผู้ใช้" });
    }
